...
model, resolution = get_model(framework='tf', model_variant='rt_lite')
...
def interference(filename: str) -> List[Tuple[str, int, int]]:
    ...
    """
    return analyze_image(model=model, file_path=filename, framework='tf', resolution=resolution, lite=True)
